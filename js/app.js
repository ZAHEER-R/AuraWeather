(() => {
  ...
  /* Profile → small Download APK button (no popup on app open) */
  function wireDownloadApk() {
    $('#download-apk-btn')?.addEventListener('click', async () => {
      try {
      ...
      showToast('APK not available yet — place AuraWeather.apk in the app folder to enable download');
    });
  }
