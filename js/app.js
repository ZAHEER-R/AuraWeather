(() => {
  ...
  async function loadNews() {
    ...
    if (status) status.textContent = articles[0]?.source === 'Aura Digest' ? 'Sample feed · 24/7' : 'Live · updates 24/7';
    if (list) {
      list.innerHTML = articles.map(a => {
        const thumb = a.image
          ? `<img class="news-thumb" src="${esc(a.image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src='';this.classList.add('broken');this.parentElement.querySelector('.news-thumb-fallback')?.classList.remove('hidden')">`
          : `<div class="news-thumb placeholder">📰</div>`;
        const fallback = a.image ? `<div class="news-thumb placeholder news-thumb-fallback hidden">📰</div>` : '';
        return `<a class="news-card" href="${a.url && a.url !== '#' ? a.url : 'javascript:void(0)'}" ${a.url && a.url !== '#' ? 'target="_blank" rel="noopener"' : ''}>
          <div class="news-thumb-wrap">${thumb}${fallback}</div>
          <div class="news-body"><b>${esc(a.title)}</b><p>${esc(a.desc || '')}</p>
          <span class="news-meta">${esc(a.source)} · ${a.time ? new Date(a.time).toLocaleDateString() : ''}</span></div></a>`;
      }).join('');
    }
  ...
  function updateTodayHighlights(articles) {
    ...
    const slides = top4.map((a, i) => {
      const img = a.image
        ? `<img src="${esc(a.image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none'">`
        : `<div class="th-img-ph">📰</div>`;
      return `<a class="th-slide" href="${a.url && a.url !== '#' ? esc(a.url) : 'javascript:void(0)'}" ${a.url && a.url !== '#' ? 'target="_blank" rel="noopener"' : ''} data-i="${i}">
  ...
  /* Desktop: Add to Home Screen · Mobile/tablet: Install / Download APK */
  let _deferredInstall = null;
  function isMobileOrTablet() {
    const ua = navigator.userAgent || '';
    return /Android|iPhone|iPad|iPod|Mobile|Tablet|webOS|BlackBerry|IEMobile|Opera Mini/i.test(ua)
  ...
  function wireInstallPrompt() {
    ...
    // Also show a gentle prompt after app is used (if not dismissed)
    setTimeout(() => {
      if (!sessionStorage.getItem('aura_install_dismissed')) showInstallBanner();
    }, 8000);

    $('#install-dismiss-btn')?.addEventListener('click', () => {
      $('#install-banner')?.classList.add('hidden');
      sessionStorage.setItem('aura_install_dismissed', '1');
    });
    $('#install-primary-btn')?.addEventListener('click', async () => {
      const mobile = isMobileOrTablet();
      if (mobile) {
        // Prefer PWA install; else try APK download
        if (_deferredInstall) {
          try {
            ...
            $('#install-banner')?.classList.add('hidden');
            showToast('Thanks — check your home screen');
            return;
          } catch (_) {}
        }
        // Download APK if hosted
        try {
          const res = await fetch('AuraWeather.apk', { method: 'HEAD' });
          if (res.ok) {
            const a = document.createElement('a');
            a.href = 'AuraWeather.apk';
            a.download = 'AuraWeather.apk';
            document.body.appendChild(a);
            a.click();
            a.remove();
            showToast('Downloading AuraWeather APK…');
            $('#install-banner')?.classList.add('hidden');
            return;
          }
        } catch (_) {}
        showToast('Use browser menu → Add to Home Screen, or host AuraWeather.apk for download');
      } else {
        ...
        if (_deferredInstall) {
          try {
            ...
            _deferredInstall = null;
            $('#install-banner')?.classList.add('hidden');
            showToast('AuraWeather can be pinned from your browser');
            return;
          } catch (_) {}
        }
        showToast('Browser menu → Install app / Add to Home Screen (Windows & macOS)');
      }
    });
  }
  function showInstallBanner() {
    if (sessionStorage.getItem('aura_install_dismissed')) return;
    const banner = $('#install-banner');
    const title = $('#install-title');
    const desc = $('#install-desc');
    const btn = $('#install-primary-btn');
    if (!banner) return;
    const mobile = isMobileOrTablet();
    if (mobile) {
      if (title) title.textContent = 'Get AuraWeather on your device';
      if (desc) desc.textContent = 'Install the app widget or download the Android APK for offline-ready access.';
      if (btn) btn.textContent = 'Install / Download app';
    } else {
      if (title) title.textContent = 'Add AuraWeather to Home Screen';
      if (desc) desc.textContent = 'Install on Windows or macOS for a full desktop app experience.';
