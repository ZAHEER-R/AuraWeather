(() => {
    ...
    if (Auth.isLoggedIn()) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (wrap) {
            // Make GIS button full width / larger
            wrap.style.width = '100%';
            wrap.querySelectorAll('div').forEach(d => { d.style.width = '100%'; });
          }
          if (fb) {
  ...
  async function loadPlace(place) {
    ...
    // Persist every visited place so switching never erases history
    if (place && place.lat != null && place.lon != null) {
      Store.saveCity(place);
    }
    try {
      ...
      bars.innerHTML += `
        <div class="hour-col" data-hour="${i}">
          <div class="hour-content">
            ...
            <div class="h-rain-indicator" style="opacity:${0.35 + precipProb / 100 * 0.65}">
  ...
  function renderWeatherChart(mode) {
    ...
    const daily = state.forecast?.daily;
    if (!hourly && !daily) {
      svg.innerHTML = '<text x="320" y="120" text-anchor="middle" fill="rgba(255,255,255,0.5)" font-size="14">No data</text>';
      return;
    }
    ...
    function gridY(minV, maxV, ticks, unitFmt) {
      ...
      for (let t = 0; t <= ticks; t++) {
        ...
        g += `<line x1="${padL}" y1="${y.toFixed(1)}" x2="${W - padR}" y2="${y.toFixed(1)}" stroke="rgba(255,255,255,0.06)" stroke-width="1"/>`;
        g += `<text x="${padL - 6}" y="${(y + 4).toFixed(1)}" text-anchor="end" fill="rgba(255,255,255,0.45)" font-size="11" font-family="Space Grotesk,sans-serif">${unitFmt(val)}</text>`;
      }
      return g;
    }
    ...
    function xLabelsHourly() {
      ...
      for (let i = 0; i < 24; i += 2) {
        s += `<text x="${xAt(i, 24).toFixed(1)}" y="${H - 12}" text-anchor="middle" fill="rgba(255,255,255,0.45)" font-size="11" font-family="Space Grotesk,sans-serif">${String(i).padStart(2,'0')}:00</text>`;
      }
      return s;
    }
    ...
    if (mode === 'temp') {
      ...
      times.forEach((day, i) => {
        ...
        const x = xAt(i, n);
        body += `<text x="${x.toFixed(1)}" y="${H - 12}" text-anchor="middle" fill="rgba(255,255,255,0.45)" font-size="10" font-family="Space Grotesk,sans-serif">${short}</text>`;
        const v = uvs[i];
        if (v == null) return;
  ...
  function setHealthPanelOpen(open) {
    ...
    if (open) {
      // Smooth expand — scroll form into view slightly
      setTimeout(() => {
  ...
  function placeOnArc(id, t) {
    // Semicircle from (10,90) to (190,90), radius 90, center (100,90)
  ...
  async function loadNews() {
    ...
    if (list) {
      list.innerHTML = articles.map(a => {
        const thumb = a.image
          ? `<img class="news-thumb" src="${esc(a.image)}" alt="" loading="lazy" onerror="this.style.display='none'">`
  ...
  function renderPlacesStrip() {
    ...
    cities.forEach((c, i) => {
      ...
      html += `<button type="button" class="place-pill${active ? ' active' : ''}" data-i="${i}">${esc(c.name || c.label)}</button>`;
    });
  ...
  function renderSavedCities() {
    ...
    // Mobile/tablet: collapse to ~2 lines unless expanded
    if (moreBtn) {
      const needsMore = cities.length > 4;
      moreBtn.style.display = needsMore ? 'inline-flex' : 'none';
      ...
      moreBtn.onclick = () => {
        ...
        // classList.toggle returns true if class was added
        const isCollapsed = el.classList.contains('collapsed');
        moreBtn.textContent = isCollapsed ? 'See more' : 'Show less';
        el.dataset.expanded = isCollapsed ? '' : '1';
      };
    }
  }
