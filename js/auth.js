...
const Auth = (() => {
  ...
  function initGoogleButton(containerId, onSuccess, onError) {
    ...
    try {
      ...
      if (el) {
        const card = el.closest('.auth-card') || el.parentElement;
        const parentW = (card && card.clientWidth) || el.clientWidth || 280;
        const available = Math.max(200, Math.floor(parentW - 48));
        const btnWidth = Math.min(280, available);
        el.style.width = '100%';
