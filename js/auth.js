...
const Auth = (() => {
  ...
  function initGoogleButton(containerId, onSuccess, onError) {
    ...
    try {
      ...
      if (el) {
        // Use container width so the button stays inside the card on mobile
        const btnWidth = Math.min(360, Math.max(240, Math.floor(el.clientWidth || el.parentElement?.clientWidth || 280)));
