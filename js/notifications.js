...
const Notify = (() => {
  ...
  function personalAlert(placeName, summary) {
    const settings = Store.getSettings();
    if (!settings.alerts || settings.personalAlerts === false) return;
    show(`AuraWeather · Personal · ${placeName}`, {
      body: summary,
      tag: 'personal-' + (placeName || 'loc'),
      requireInteraction: false
    });
  }

  function travelAlert(placeName, summary) {
    const settings = Store.getSettings();
    if (!settings.alerts || settings.personalAlerts === false) return;
    show(`AuraWeather · Travel safety · ${placeName}`, {
      body: summary,
      tag: 'travel-' + (placeName || 'loc'),
  ...
  return {
    ...
    weatherAlert, newsAlert, disasterAlert, personalAlert, travelAlert, registerFCM
  };
})();
